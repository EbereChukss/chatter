...
const Auth: React.FC<AuthProps> = ({ modal, setModal }) => {
  ...
  const googleAuth = async () => {
    try {
      const createUserResult = await signInWithPopup(auth, provider);
  ...
  return (
    <Modal modal={modal} setModal={setModal}>
      <section
        className={`z-50 fixed top-0 bottom-0 left-0 md:left-[10rem] overflow-auto right-0 md:right-[10rem] bg-white shadows transition-all duration-500 ${
