...
// Define the types for comment and postId
...
const Comment: React.FC<CommentProps> = ({ item: comment, postId }) => {
  ...
  const removeComment = async () => {
    try {
      ...
      const typedError = error as Error; // Type assertion to access error properties safely
      toast.error(typedError.message || "Failed to remove comment");
    }
  };
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      toast.error(typedError.message || "Failed to update comment");
    } finally {
      setLoading(false);
    }
  };
